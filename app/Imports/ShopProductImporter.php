...
class ShopProductImporter implements OnEachRow, WithValidation, WithHeadingRow
{
    use Importable;
    ...
    public function onRow(Row $rowData)
    {
        ...
        if (isset($row['id']) && $product = Product::find($row['id'])) {
            if ($product->shop_id != $this->shop->id) throw ValidationException::withMessages(['Invalid ID on row ' . ($rowIndex + 1)]);
        }
        if (isset($row['parent_id']) && $parentProduct = Product::find($row['parent_id'])) {
            if ($parentProduct->shop_id != $this->shop->id) throw ValidationException::withMessages(['Invalid Parent ID on row ' . ($rowIndex + 1)]);
        }
