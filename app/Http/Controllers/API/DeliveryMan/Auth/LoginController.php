...
class LoginController extends Controller
{
    ...
    */
    ...
    public function socialLogin(Request $request)
    {
        ...
        try {
            $user = DeliveryMan::query()->whereHas('socialAccounts', function ($q) use ($request) {
                /* @var Builder $q */
            ...
            if (!$user) {
                $request->validate([
                    'name' => 'required|string|max:191',
                    'email' => 'nullable|email|max:191|unique:delivery_men,email'
                ]);
                $n = $request->name;
                $u = Str::slug($n) . '_' . uniqid();
        ...
        if ($request->device_token) {
            try {
                if (filter_var($request->username, FILTER_VALIDATE_EMAIL)) {
                    DeliveryMan::query()->where('email', $request->username)->where('status', 1)->update([
                        'device_token' => $request->device_token
                    ]);
                } else {
                    DeliveryMan::query()->where($this->username(), $request->username)->where('status', 1)->update([
                        'device_token' => $request->device_token
                    ]);
                }
            } catch (\Exception $exception) {
