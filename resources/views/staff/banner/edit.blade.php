...
@section('content')
    ...
    <div id="app">
        ...
        <form action="{{ route('staff.setting.banner.update', $banner->id) }}" method="post">
            @csrf
            @method('PUT')
                <div class="row">
                    <div class="col-md-12">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <img class="img-thumbnail img-fluid"
                                         src="{{ $banner->image??'https://picsum.photos/500/200' }}" id="banner-preview">
                                    <input type="hidden" name="image" value="{{ $banner->image??'https://picsum.photos/500/200' }}" id="banner">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="title" name="title" class="form-control" placeholder="Promo Banner" value="{{ $banner->title }}">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="subtitle">Subtitle</label>
                                            <input type="text" id="subtitle" name="subtitle" class="form-control" placeholder="Winter Offer" value="{{ $banner->subtitle }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="tags[]" class="form-control" id="tags" multiple>
                                                @foreach($tags as $tag)
                                                    <option
                                                            value="{{ $tag->id }}" {{ (is_array($oldTags) && in_array($tag->id, $oldTags))?'selected':'' }}>{{ $tag->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group">
                                    <button class="btn btn-block btn-success" type="submit">UPDATE BANNER</button>
                                </div>
                            </div>
                        </div>
                    </div>
