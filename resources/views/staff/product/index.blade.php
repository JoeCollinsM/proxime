...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header"><i class="fa fa-table"></i> All Products
            <div class="btn-group float-sm-right">
            <a class="btn btn-success btn-sm  waves-effect waves-light" href="{{ route('staff.catalog.product.create') }}"><i class="fa fa-plus mr-1"></i> Add new</a>
            <button id="import" class="btn btn-info btn-sm"><i
                                    class="fa fa-cloud-upload"></i> Import
                        </button>
            <a class="btn btn-outline-info btn-sm  waves-effect waves-light" href="{{ asset('staff/proxime-product-import-template.xlsx') }}"><i class="fa fa-cloud-download"></i> Import Template</a>
            </div>  
            </div>
    ...
    <form id="import-form" action="{{ route('staff.catalog.product.import') }}" class="d-none" method="POST">
        @csrf
        ...
        (function ($) {
            $(document).ready(function () {
                $('#data-table').DataTable({
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: '{{ route('staff.catalog.product.index') }}',
                    columns: [
                        {data: 'id', name: 'id'},
            ...
            $(document).on('click', '.btn-delete', function (e) {
                e.preventDefault();
                var id = $(this).data('id');
                var url = '{{ url('staff/catalog/product') }}/' + id
