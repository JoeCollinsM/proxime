...
@section('content')
    ...
    <div id="app">
        ...
        <div class="row">
            <div class="col-lg-12">
                <form action="{{ route('staff.catalog.order.store') }}" method="post">
                    @csrf
                    <div class="card">
                        <div class="card-body">
    ...
    <script>
        ...
        window.app = new Vue({
            ...
            data: {
                ...
                customer: @json(old('customer')??null),
                product_id: @json(old('product_id')??null),
                shop_id: @json(old('shop_id')??null),
                shipping_method_id: @json(old('shipping_method_id')??null),
                billing: @json(is_array(old('billing'))?old('billing'):null),
                shipping: @json(is_array(old('shipping'))?old('shipping'):null),
                items: @json(is_array(old('items'))?old('items'):[]),
                shipping_method: null,
                coupon_id: @json(old('coupon_id')??null),
                coupon: null,
            },
            beforeMount() {
                if (!this.customer) {
                ...
                if (!this.shipping_method && this.shipping_method_id) {
                    if (this.shipping_methods.length) {
                        var filter = this.shipping_methods.filter(sh => {
                            return parseInt(sh.id) === parseInt(this.shipping_method_id)
                        })
                        if (filter.length) {
                            this.shipping_method = filter[0]
                        }
                    }
                }
                if (!this.coupon && this.coupon_id) {
                    if (this.coupons.length) {
                        var filter = this.coupons.filter(cou => {
                            return parseInt(cou.id) === parseInt(this.coupon_id)
                        })
                        if (filter.length) {
                            var coupon = filter[0]
