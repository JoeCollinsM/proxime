...
@section('page_title', 'Edit Shop Category')

@section('css_libs')
    <link rel="stylesheet" href="{{ asset('staff/css/select2.min.css') }}">
    <link rel="stylesheet" href="{{ asset('staff/css/select2-bootstrap.css') }}">
@endsection
...
@section('content')
    ...
    <form action="{{ route('staff.shop-category.update', $shopCategory->id) }}" method="post">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" value="{{ $shopCategory->name }}"
                                    class="form-control">
                        </div>
                        <div class="form-group">
                            <label class="control-label">Category Slug</label>
                            <input type="text" name="slug" value="{{ $shopCategory->slug }}"
                                    class="form-control">
                        </div>
                        <div class="form-group">
                            ...
                            <select name="parent_id" class="form-control select2">
                                ...
                                @foreach($categories as $category)
                                    <option
                                        value="{{ $category->id }}" {{ $shopCategory->parent_id == $category->id?'selected':'' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control select2">
                                <option value="1" {{ $shopCategory->status == 1?'selected':'' }}>Enable</option>
                                <option value="0" {{ $shopCategory->status == 0?'selected':'' }}>Disable
                                </option>
                            </select>
                        </div>
                        <div class="form-group">
                            <div>
                                <img src="{{ $shopCategory->image??'//via.placeholder.com/200x200' }}"
                                        id="add-image-preview">
                            </div>
                            <input type="hidden" name="image" id="add-image-input"
                                    value="{{ $shopCategory->image }}">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="meta[title]" value="{{ $shopCategory->meta['title'] }}"
                                    class="form-control">
                        </div>
                        <div class="form-group">
                            <label class="control-label">Meta Description</label>
                            <textarea name="meta[description]"
                                        class="form-control">{{ $shopCategory->meta['description'] }}</textarea>
                        </div>
                        <div class="form-group">
                            <label class="control-label">Meta Keywords (separate by comma)</label>
                            <input type="text" name="meta[keywords]"
                                    value="{{ $shopCategory->meta['keywords'] }}"
                                    class="form-control">
                        </div>
                        <div class="form-group">
                            <img src="{{ $shopCategory->meta['og_image']??'//via.placeholder.com/500x200' }}"
                                    id="og-image-preview" class="img-fluid">
                            <input type="hidden" id="og-image-input" name="meta[og_image]"
                                    value="{{ $shopCategory->meta['og_image'] }}"
                                    class="form-control">
